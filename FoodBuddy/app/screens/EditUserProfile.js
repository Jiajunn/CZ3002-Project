...
const EditUserProfile = () => {
  ...
  const [arthritis, setArthritis] = useState(
    chronicDiseases.includes("Arthritis")
  );
  const [cancer, setCancer] = useState(chronicDiseases.includes("Cancer"));
  const [alzeimers, setAlzeimers] = useState(
    chronicDiseases.includes("Alzeimers")
  );
  const [osteoporosis, setOsteoporosis] = useState(
    chronicDiseases.includes("Osteoporosis")
  );
  const [diabeties, setDiabeties] = useState(
    chronicDiseases.includes("Diabetes")
  );
  const [heartDiesease, setHeartDisease] = useState(
    chronicDiseases.includes("Heart Disease")
  );
  const [respiratoryDiesease, setRespiratoryDisease] = useState(
    chronicDiseases.includes("Respiratory Disease")
  );
  ...
  const showAlert = () => {
    Alert.alert("Alert", "Confirm update details?", [
      {
        text: "Cancel",
        onPress: () => {},
        style: "cancel",
      },
      {
        ...
        onPress: () => {
          fetch(`http://${IpAddress}:8080/api/user/${userId}/update`, {
            ...
            body: JSON.stringify({
              height: parseInt(height, "decimal"),
            ...
            <View style={{ paddingBottom: 50 }}>
              ...
              <View style={{ flexDirection: "row" }}>
                <SmokingCheckBox
                  ...
                  isChecked={yes}
                  setCheckbox={setYes}
                  handleClick={handleSmokingClick}
                  isOtherChecked={no}
                  setIsOther={setNo}
